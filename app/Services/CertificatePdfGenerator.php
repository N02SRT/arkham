...
class CertificatePdfGenerator
{
    ...
    public function generate(string $pdfAbs, string $name, string $orderNo, array $barcodes = []): void
    {
        ...
        if (!is_writable($fontsDir)) {
            Log::error('CertificatePdfGenerator: fonts directory not writable', ['dir' => $fontsDir]);
            throw new \RuntimeException("TCPDF fonts directory not writable: {$fontsDir}");
        }
        if (!is_writable($cacheDir)) {
            Log::error('CertificatePdfGenerator: cache directory not writable', ['dir' => $cacheDir]);
        ...
        if (!is_writable($tempDir)) {
            Log::error('CertificatePdfGenerator: system temp directory not writable', ['temp_dir' => $tempDir]);
